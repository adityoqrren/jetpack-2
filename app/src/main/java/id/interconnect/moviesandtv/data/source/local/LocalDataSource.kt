...
class LocalDataSource private constructor(private val mMovieTVDao: MovieTVDao) {
    ...
    fun insertPopularTV(listPopularTv: List<TVItemEntity>) =
        mMovieTVDao.insertPopularTV(listPopularTv)
    ...
    fun getPopularMovies(query: SimpleSQLiteQuery): DataSource.Factory<Int, MovieItemEntity> =
        mMovieTVDao.getPopularMovies(query)
    ...
    fun getFavoriteMovies(): DataSource.Factory<Int, MovieItemEntity> =
        mMovieTVDao.getFavoriteMovies()

    fun insertPopularMovie(listPopularMovies: List<MovieItemEntity>) =
        mMovieTVDao.insertPopularMovies(listPopularMovies)
